...
const PNG_SCALES = [1, 2, 4, 8] as const;
...
export function SaveAsDialog({ suggestedName, onSave }: SaveAsDialogProps) {
  ...
  function handleSubmit(e: FormEvent) {
    ...
    onSave(name.trim(), format, pngScale);
      ...
      <dialog ref={dialogRef}>
        <form onSubmit={handleSubmit}>
          ...
          {format === "png" && (
            <label style={{ display: "block", marginBottom: 8 }}>
              scale
              <select value={pngScale} onChange={(e) => setPngScale(Number(e.target.value))} style={{ marginLeft: 8 }}>
                {PNG_SCALES.map((s) => (
